...
def _download_audio(location):
    ...
    datapath  = location
    target    = datapath #May be used to append an extra dir level here.
    targetwav = path.join(target,'wav')
    recitors  = ['Husary_64kbps'] #May be replaced by other recitors!
...
def _preprocess_data(location, amount):
    ...
    with open(path.join(location, "quran-uthmani.txt"), encoding="utf8") as f:
        for line in f:
    ...
    for root, dirnames, filenames in os.walk(targetwav):
        for filename in fnmatch.filter(filenames, "*.wav"):
            ...
            if len(trans) > trans_max:
                trans_max = len(trans)
            if len(trans) < trans_min:
                trans_min = len(trans)
            if aya_num%10 > 0:
    ...
    print (f"Transcript sizes (in char) vary from {trans_min} to {trans_max}")
    ...
    df_train = pd.DataFrame(a, columns=['transcript', 'wav_filename'], dtype=int)
    #df_dev = pd.DataFrame(b, columns=['wav_filename', 'wav_filesize', 'transcript'], dtype=int)
    df_test = pd.DataFrame(c, columns=['transcript', 'wav_filename'], dtype=int)

    df_all.to_csv(target+"/quran_all.csv", sep=',', header=True, index=False)
    df_train.to_csv(target+"/quran_train.csv", sep=',', header=False, index=False)
    #df_dev.to_csv(target+"/quran_dev.csv", sep=',', header=True, index=False)
    df_test.to_csv(target+"/quran_test.csv", sep=',', header=False, index=False)
